...
function _cons_to_prim_euler_2D_ideal_gas(U::Vector{T}, gamma::T) where {T<:Real}
    rho = U[1]
    rhou = U[2]
    rhov = U[3]
    E = U[4]
...
function _prim_to_cons_euler_2D_ideal_gas(W::Vector{T}, gamma::T) where {T<:Real}
    rho = W[1]
    u = W[2]
    v = W[3]
    p = W[4]
...
function flux(eq::Euler2D, UL::Vector{Float64}, UR::Vector{Float64}, normal::NTuple{2, Float64})::Vector{Float64}
    ...
    if eq.solver == :exact
        rho, uh, p = solve_riemann_exact(0.0,
                                         [WL[1], WL[2] * nx + WL[3] * ny, WL[4]], #projecting WL and WR on the normal
                                         [WR[1], WR[2] * nx + WR[3] * ny, WR[4]],
                                         eq.gamma)
    else
